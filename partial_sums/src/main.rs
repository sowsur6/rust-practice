...
pub fn parts_sums(arr: &[u64]) -> Vec<u64> {
    let mut partial_sums: Vec<u64> = vec![0; arr.len() + 1]; 

    for (i, _num) in arr.iter().rev().enumerate() {

        let sum = arr[0..i+1].iter().sum();

        partial_sums[arr.len() - i - 1] = sum;
    }
